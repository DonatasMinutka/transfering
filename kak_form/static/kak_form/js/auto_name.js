document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateName() {
        ...
        if (count(tenant) > 1) {
            const legalForms = [
                'UAB', 'AB', 'MB', 'VšĮ', 'IĮ', 'ŽŪB', 'VĮ', 'OOO', 'TOV', 'SIA', 'AS', 'APB',
                'UADBB', 'LLC', 'OU', 'PĮ', 'LTD', 'BĮ', 'APB', 'Individuali įmonė',
                'Uždaroji akcinė bendrovė', 'Viešoji įstaiga', 'Valstybės įmonė', 'Akcinė bendrovė',
                'Obščestvo s ograničennoj otvetstvennostju', 'Tovarystvo z obmeženoyu vidpovidalnistyu',
                'Mažoji bendrija', 'Sabiedrība ar ierobežotu atbildību', 'Žemės ūkio bendrovė',
                'Advokatų profesinė bendrija', 'Biudžetinė įstaiga', 'Limited', 'Asociacija',
                'Privati įmonė', 'Osaühing', 'Limited Liability Company',
                'Uždaroji akcinė draudimo brokerių bendrovė'
            ];
            ...
            const phrasePattern = new RegExp(
                legalForms
                    ...
                    .map(f => removeLithuanianDiacritics(f).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
                    .join('|'),
                'gi'
            );
            tenant = tenant.replace(phrasePattern, ' ');

            const singleForms = new Set(
                legalForms
                    .filter(f => !f.includes(' '))
